...
const Hero = () => {
    ...
    const interval = setInterval(() => {
      setTickerPrices((prev) => {
        ...
        Object.keys(newPrices).forEach((key) => {
          const delta =
            (Math.random() - 0.48) * (key === "XAU/USD" ? 0.8 : 0.0003);
          const newPrice = parseFloat(
            (newPrices[key].price + delta).toFixed(key === "XAU/USD" ? 0 : 4),
          );
          const changePercent = ((delta / newPrices[key].price) * 100).toFixed(
            2,
          );
          newPrices[key] = {
            price: newPrice,
  ...
  return (
    <section className="relative min-h-screen md:pt-10  mb-12">
      {/* Background gradient */}
        ...
        <div className="grid lg:grid-cols-2 gap-12 items-center min-h-[calc(100vh-76px)]">
          ...
          >
            ...
            <p className="text-lg text-muted-foreground leading-relaxed mb-8 max-w-lg">
              I'm {siteData.traderName} — professional forex trader, market
              analyst, and educator. I teach serious traders how to think
              clearly, manage risk intelligently, and build consistent profit.
            </p>
              ...
              <div className="flex gap-4">
                {Object.entries(tickerPrices).map(([pair, data]) => (
                  <div key={pair} className="text-center px-2">
                    ...
                    <div
                      className={`text-xs font-medium ${data.direction === "up" ? "text-green-500" : "text-red-500"}`}
                    >
